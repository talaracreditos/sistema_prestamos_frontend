...
import { ClockIcon, SunIcon, ArrowsRightLeftIcon } from '@heroicons/react/24/outline';
        ...
        <form onSubmit={handleSubmit}
            ...
            <div>
                <label className="block text-[10px] font-black text-slate-400 dark:text-dark-text-muted uppercase tracking-widest mb-2 flex items-center gap-1 transition-colors">
                    <ArrowsRightLeftIcon className="w-3 h-3" /> Turno *
                </label>
                <div className="flex gap-2 flex-wrap">
                    {TURNOS.map(t => {
                        const active = formData.turno === t.value;
                        return (
                            <button key={t.value} type="button" onClick={() => handleChange('turno', t.value)}
                ...
                <p className="text-[9px] text-slate-400 dark:text-dark-text-muted/70 font-bold mt-1.5 transition-colors">
                    Usa "Mañana" y "Tarde" cuando el mismo rol tenga turno partido en el mismo día (ej: 8:30–1:30 y 2:30–8:30pm). Crea un horario separado para cada bloque.
                </p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="time" value={formData.hora_inicio}
                        onChange={e => handleChange('hora_inicio', e.target.value.slice(0, 5))}
                        required
                        className="w-full border border-slate-300 dark:border-dark-border rounded-xl px-4 py-2.5 text-sm font-bold
                            focus:ring-2 focus:ring-brand-red dark:focus:ring-brand-gold outline-none transition-all
                            bg-white dark:bg-dark-surface-alt text-slate-800 dark:text-dark-text"
                ...
                <div>
                    ...
                    <input type="time" value={formData.hora_fin}
                        onChange={e => handleChange('hora_fin', e.target.value.slice(0, 5))}
                        required
                        className="w-full border border-slate-300 dark:border-dark-border rounded-xl px-4 py-2.5 text-sm font-bold
                            focus:ring-2 focus:ring-brand-red dark:focus:ring-brand-gold outline-none transition-all
                            bg-white dark:bg-dark-surface-alt text-slate-800 dark:text-dark-text"
            ...
            <div className="pt-2">
                <button type="submit"
                    disabled={loading || !formData.nombre || !formData.hora_inicio || !formData.hora_fin || !formData.turno || formData.dias.length === 0}
