...
const AbrirSesionModal = ({ isOpen, onClose, onConfirm, loading }) => {
    ...
    useEffect(() => {
        if (isOpen) {
            setCajaId(''); setMontoApertura(''); setObservaciones(''); setPin(''); setCajaKey(Date.now());
        }
    }, [isOpen]);

    const handleSubmit = (e) => {
        e.preventDefault();
        onConfirm({ caja_id: cajaId, monto_apertura: montoApertura || 0, observaciones, pin });
    };

    const canSubmit = cajaId && montoApertura !== '' && pin.length === 6;

    return (
        <ViewModal isOpen={isOpen} onClose={onClose} hideFooter={true} title="Aperturar Caja Chica">
            <form onSubmit={handleSubmit} className="space-y-5 transition-colors">
                <div>
                    <label className="block text-[10px] font-bold text-slate-400 dark:text-dark-text-muted uppercase mb-2">Seleccionar Caja *</label>
                    <CajaSearchSelect key={cajaKey} onSelect={(caja) => setCajaId(caja ? caja.id : '')} disabled={loading} />
                </div>

                <div>
                    <label className="block text-[10px] font-bold text-slate-400 dark:text-dark-text-muted uppercase mb-2">Monto de Apertura *</label>
                ...
                <div>
                    ...
                    <div className="relative">
                        ...
                        <textarea
                            ...
                            placeholder="Ej: Se entrega para cubrir gastos del mes..."
                ...
                <div>
                    <label className="block text-[10px] font-bold text-slate-400 dark:text-dark-text-muted uppercase mb-2 flex items-center gap-1">
                        <KeyIcon className="w-3.5 h-3.5" /> PIN de Autorización *
                    </label>
                        ...
                        {Array.from({ length: 6 }).map((_, i) => (
                            <input
                                ...
                                onChange={(e) => {
                                    ...
                                    setPin(next.join(''));
                                    if (val) e.target.parentElement.children[i + 1]?.focus();
                                }}
                                onKeyDown={(e) => {
                                    if (e.key === 'Backspace' && !pin[i] && i > 0) e.target.parentElement.children[i - 1]?.focus();
                                }}
                                onPaste={(e) => {
                    ...
                    <p className="text-[9px] text-slate-400 dark:text-dark-text-muted font-medium text-center mt-2">
                        Solicita el PIN a un administrador para aperturar la caja chica.
                    </p>
                </div>
                ...
                >
                    {loading ? 'Procesando...' : 'Abrir Caja Chica'}
                </button>
            </form>
        </ViewModal>
    );
