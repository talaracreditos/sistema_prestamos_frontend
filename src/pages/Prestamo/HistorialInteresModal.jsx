...
const HistorialInteresModal = ({ isOpen, onClose, data }) => {
    if (!isOpen || !data) return null;

    const totalReducido = (data.historial ?? []).reduce((s, h) => s + Math.abs(parseFloat(h.monto_reducido ?? 0)), 0);

    // Se renderiza en document.body (portal) para escapar de cualquier
    // contenedor con transform / z-index (p. ej. el animate-in de OperacionForm)
    // que hacía que el overlay no cubriera toda la pantalla.
    return createPortal(
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-slate-900/50 dark:bg-black/80 backdrop-blur-sm animate-in fade-in duration-200 transition-colors">
            <div className="bg-white dark:bg-dark-surface rounded-[24px] shadow-2xl dark:shadow-black/50 w-full max-w-sm overflow-hidden border border-slate-100 dark:border-dark-border animate-in zoom-in-95 duration-200 transition-colors">
                ...
                <div className="p-5 bg-slate-50 dark:bg-dark-surface-alt border-b border-slate-100 dark:border-dark-border flex items-center justify-between transition-colors">
                    <h3 className="font-black text-slate-800 dark:text-dark-text uppercase text-xs tracking-widest transition-colors">
                        Historial Interés — Cuota #{data.nro}
                    </h3>
                ...
                <div className="p-5">
                    <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
                        {(data.historial ?? []).map((h, i) => {
                            const monto = Math.abs(parseFloat(h.monto_reducido ?? 0));

                            return (
                                <div key={i} className="flex items-start justify-between text-xs p-3 rounded-xl border gap-2 bg-green-50 dark:bg-green-500/10 border-green-200 dark:border-green-500/20 transition-colors">
                                    <div className="flex items-start gap-2 min-w-0">
                                        <div className="mt-0.5 p-1 rounded-full shrink-0 bg-green-100 dark:bg-green-500/20">
                                            <ArrowDownIcon className="w-3 h-3 text-green-600 dark:text-green-400" />
                                        </div>
                                        <div className="min-w-0">
                                            <p className="font-bold truncate text-green-700 dark:text-green-400">
                                                {h.created_at ? new Date(h.created_at).toLocaleDateString() : ''}
                                            </p>
                                            <p className="text-[9px] font-black uppercase mt-0.5 text-green-500 dark:text-green-400">
                                                Reducción aplicada
                                            </p>
                                            {h.motivo && (
                                    ...
                                    <div className="text-right shrink-0">
                                        ...
                                        <p className="text-[9px] font-bold text-green-500 dark:text-green-400">
                                            Quedó en: S/ {parseFloat(h.interes_nuevo ?? 0).toFixed(2)}
                                        </p>
                                    </div>
                                </div>
                            );
                    ...
                    <div className="mt-4 pt-4 border-t border-slate-100 dark:border-dark-border space-y-1.5 transition-colors">
                        <div className="flex items-center justify-between">
                            ...
                            <span className="text-sm font-black text-green-600 dark:text-green-400 line-through">-S/ {totalReducido.toFixed(2)}</span>
                        </div>
                        <div className="flex items-center justify-between pt-1 border-t border-slate-100 dark:border-dark-border transition-colors">
                            <span className="text-[10px] font-black uppercase text-slate-500 dark:text-dark-text-muted">Interés pendiente actual</span>
                            <span className="text-lg font-black text-brand-red dark:text-red-400">S/ {parseFloat(data.total ?? 0).toFixed(2)}</span>
                        </div>
                    </div>
                </div>
            </div>
...
export default HistorialInteresModal;